...
def main():
    ...
    return
    preprocessor = SDPWarmstart()
    solver = QAOASolver(data, JobShopHamiltonianConstructor(), 6, 1, variable_pruning=True, objective_bias=1)
    cplex_solver = CPLEXSolver(solver.get_data())
    cplex_solver.solve()
    optimal_plottable_solution = cplex_solver.get_plottable_solution()
    solver.solve(optimal_plottable_solution=optimal_plottable_solution)
    return
    solver.plot_expectation_heatmap((128, 128), 1000)
    plt.show()


    def to_solution_filepath(p, rep):
        return "Micro_QAOA_results" + os.sep + "QAOA_" + str(p) + '_' + str(rep) + '.txt'
    ...
    for rep in range(1, 100):
        for p in range(1, 11):
            ...
            solver.reset_theta()
            solver.solve(optimal_plottable_solution=optimal_plottable_solution)
            solver.store_solution(to_solution_filepath(p, rep))
            print(p, rep)
    return

    solver2 = QAOASolver(data, hamiltonian_constructor, 5, 4, variable_pruning=True, objective_bias=0)

    compare_qaoa_versions("SDP_Experiments_p4" + os.sep, [solver2], [4], num_samples_per_setup=1000, solver_names=["QAOA"])
    plt.show()
