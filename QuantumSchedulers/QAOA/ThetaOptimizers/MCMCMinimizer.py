...
class MCMCMinimizer(ThetaOptimizer):
    ...
    def optimize_theta(self, circuit_builder: CircuitBuilder, qc_sampler: QCSampler, num_reads: int, hamiltonian,
        ...
        while self._steps < self._mcmc_steps:
            print("Theta ", self._steps, self._theta_t, "Lambda_t", self._lambda_t, "Grad_lambda_t",
                  self._grad_lambda_t,
    ...
    def acceptance_distribution(self, theta_d: np.ndarray, lambda_d: float,
        ...
        g_d_given_t = np.prod(
            [self._proposal_distributions_t[k].pdf(self._theta_t[k] - theta_d[k]) for k in range(theta_d.size)])
        g_t_given_d = np.prod(
            [proposal_distributions_d[k].pdf(theta_d[k] - self._theta_t[k]) for k in range(theta_d.size)])
        ...
        return np.minimum(1,
                          self.p_boltzmann(lambda_d) * g_t_given_d / (self.p_boltzmann(self._lambda_t) * g_d_given_t))
